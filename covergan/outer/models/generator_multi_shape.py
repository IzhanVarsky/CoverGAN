...
class GeneratorMultiShape(nn.Module):
    def __init__(self, z_dim: int, audio_embedding_dim: int, has_emotions: bool, num_layers: int, canvas_size: int,
        ...
        super(GeneratorMultiShape, self).__init__()
        path_count = 3
        self.path_depth = 3
        ...
        # self.figures_class_predictor = torch.nn.Sequential(
        #     nn.Linear(in_features=self.no_random_in_features, out_features=len(self.fig_classes)),
        # )
    ...
    def my_mega_forward(self, noise: torch.Tensor, audio_embedding: torch.Tensor,
        ...
        figures_count = np.ones(len(noise)).astype('int32') * self.base_figures_count
        ...
        for b_idx, cur_batch_fig_count in enumerate(figures_count):
            ...
            for fig_num in range(cur_batch_fig_count):
                ...
                rand_class = random.randint(0, len(self.fig_classes) - 1)
                rand_class_data = self.fig_classes[rand_class]
                if rand_class_data[0] != "oval" and rand_class_data[0] != "rect":
                    radius = radius[None, 0]
                init_fig_points = rand_class_data[1](center_point, radius, angle, self.path_depth).flatten()

                if self.RANDOM_DEFORMATION:
                    # deformation_points = torch.rand(self.one_path_points_count).to(noise.device)
                    deformation_points = torch.rand(rand_class_data[2] * (2 ** (self.path_depth - 1)) * 3 * 2).to(
                        noise.device)
                else:
                    ...
                    index += inc
                deformation_points = (deformation_points - 0.5) * 2 * radius.mean() * 0.1 * 0

                deformated_path = init_fig_points + deformation_points
