...
from scripts.filter_unicode_fonts import font_supports_all_glyphs
...
def claster(image, k=4, with_labels_centers=False):
...
def get_best_font(phrase_words, fit_func, font_size_boundaries=(2, 200), for_svg=False):
    max_tries = 50
    for test_ind in range(max_tries):
        font_name, font_path = get_random_font(for_svg)
...
def paste_horizontal_text(lst, phrase, segmented_image, for_svg=False, debug=False):
    ...
    font, font_name, fontsize = get_best_font([phrase], fit_func, for_svg=for_svg)
...
def get_random_font(for_svg=False):
    fnames = os.listdir(fonts_dir)
    if for_svg:
        fnames = list(filter(lambda x: "Condensed" not in x, fnames))
    font_name = np.random.choice(fnames)
    font_path = rf"{fonts_dir}\{font_name}"
    return font_name, font_path


def draw_rect_by_points(draw, x_left, y_top, x_right, y_bottom, r=1, color=(255, 0, 0, 255)):
    draw.ellipse(circle_point(x_left, y_top, r), color)
    draw.ellipse(circle_point(x_right, y_top, r), color)
    draw.ellipse(circle_point(x_left, y_bottom, r), color)
    draw.ellipse(circle_point(x_right, y_bottom, r), color)
...
def paste_vertical_text_by_words(lst, words, segmented_image, use_word_joining, for_svg=False, debug=False):
    result = []
    _, x_left, y_top, x_right, y_bottom, area = lst
    rect_height = x_right - x_left
    rect_width = y_bottom - y_top
    ...
    font, font_name, fontsize = get_best_font(words, fit_func, for_svg=for_svg)
    if use_word_joining:
        new_words = try_unite_words(words, font, rect_width, rect_height, height_coef)
...
def paste_text(lst, phrase, segmented_image, for_svg=False):
    if is_horizontal_lst(lst):
        return paste_horizontal_text(lst, phrase, segmented_image, for_svg=for_svg)
    if " " in phrase:
        is_letters = False
        ...
        is_letters = True
        words = phrase
    return paste_vertical_text_by_words(lst, words, segmented_image, use_word_joining=not is_letters, for_svg=for_svg)
...
def save_image(img, path, fname):
    cv2.imencode(".png", img)[1].tofile(path + f'/{fname}.png')
...
def get_all_boxes_info_to_paste(artist_name, track_name, image, for_svg=False):
    ...
    for k in ks:
        segmented_image, labels, centers = claster(image, k, with_labels_centers=True)
        ...
        for lst in results:
            if is_horizontal_lst(lst):
                result = paste_horizontal_text(lst, full_phrase, segmented_image, for_svg=for_svg)
        ...
        for lst in results:
            if not is_horizontal_lst(lst):
                result = paste_vertical_text_by_words(lst, full_phrase, segmented_image,
        ...
        def try_many_variants(lst, phrase):
            if not is_horizontal_lst(lst):
                # try by letters
                res = paste_vertical_text_by_words(lst, phrase, segmented_image,
                                                   use_word_joining=False, for_svg=for_svg)
                if not check_sizes(res):
                    if " " in phrase:
                        # try by words
                        res = paste_vertical_text_by_words(lst, phrase.split(), segmented_image,
                                                           use_word_joining=False, for_svg=for_svg)
                        if not check_sizes(res):
                ...
                res = paste_horizontal_text(lst, phrase, segmented_image, for_svg=for_svg)
            return res
        ...
        for lst in with_biggest_area:
            result = paste_vertical_text_by_words(lst, [artist_name, track_name], segmented_image,
                                                  use_word_joining=False, for_svg=for_svg)
            if check_sizes(result):
                return result
            break
        ...
        result = []
        result.extend(paste_text(lst1, artist_name, segmented_image, for_svg=for_svg))
        result.extend(paste_text(lst2, track_name, segmented_image, for_svg=for_svg))
        return result


def draw_phrases(artist_name, track_name, img_path, output_path):
    pil_image = PIL.Image.open(img_path).convert(mode='RGB')
    draw = ImageDraw.Draw(pil_image)
    to_draw = get_all_boxes_info_to_paste(artist_name, track_name, np.asarray(pil_image.copy()))
    for x in to_draw:
...
def draw_to_draw_object(draw, d):
    if d["type"] == "rect":
        ...
        draw.ellipse(circle_point(x_l + ascent, y_t, 1), d["color"])
...
if __name__ == '__main__':
    draw_phrases("KYUL", "VeryVeryVeryVeryVeryVeryLongWordHere",
                 img_path="test_img5.png", output_path="debug_out.png")
    # draw_phrases("&me", "The Rapture Pt.II", img_path="test_img.png", output_path="sample_output1.png")
    # draw_phrases("&me", "The Rapture Pt.II", img_path="test_img2.png", output_path="sample_output2.png")
    # draw_phrases("&me", "The Rapture Pt.II", img_path="test_img3.png", output_path="sample_output3.png")
    # draw_phrases("&me", "The Rapture Pt.II", img_path="test_img4.png", output_path="sample_output4.png")
    # draw_phrases("KYUL", "Even Though We Are Not the Same X X X X X XXXX XXXXXX",
    #              img_path="test_img5.png", output_path="sample_output5.png")
    # draw_phrases("Кино", "Группа Крови", img_path="test_img5.png", output_path="sample_output6.png")
