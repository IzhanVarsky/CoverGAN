...
from ..svg_tools import svg_tools


class MyGeneratorFixedFigure(nn.Module):
    def __init__(self, z_dim: int, audio_embedding_dim: int, has_emotions: bool, num_layers: int, canvas_size: int,
        ...
        super(MyGeneratorFixedFigure, self).__init__()
        path_count = 3
        self.path_depth = 3
        self.radius_coef = 0.4
        self.deform_coef = 0.15
        func_type = 3  # sets the init function (circle/oval/rect/reg_fig...)
        fig_class = svg_tools.fig_classes[func_type]
        self.init_fig_func = fig_class[0]
        self.start_path_segment_count = fig_class[1]
        self.fig_radius_count = fig_class[2]
        path_segment_count = self.start_path_segment_count * (2 ** (self.path_depth - 1))
    ...
    def my_mega_forward(self, noise: torch.Tensor, audio_embedding: torch.Tensor,
        ...
        for b_idx, shape_params in enumerate(all_shape_params):
            ...
            for _ in range(self.all_paths_count):
                ...
                deformated_path = self.init_fig_func(center_point, radius, angle, self.path_depth, deformation_points)
